...
class Reportes extends CI_Controller
{
	...
	public function reporte_stockMinimo_bs(){	
		...
		//var_dump($results);exit;
		$contenido ='<br><table class="table table-streap">
						 <tr>
						 	<th class="info">CATEGORIA</th>
							<th class="info">CODIGO</th>
							<th class="info">DESCRIPCION</th>
							<th class="success text-center">CANTIDAD</th>	
							<th class="warning text-center">STOCK MINIMO</th>
							<th class="danger text-center">REPOSICION</th>
						</tr>';

		foreach($reporteTotal  as $value) { 

			$reposicion = $value['prod_cantidad_minima'] - $value['prod_stock'];
			$contenido .=	'<tr>
								<td>'.$value['cat_nombre'].'</td>
								<td>'.$value['prod_codigo'].'</td>
								<td>'.$value['prod_nombre'].'</td>
								<td class="success text-center">'.$value['prod_stock'].'</td>
								<td class="warning text-center">'.$value['prod_cantidad_minima'].'</td>
								<td class="danger text-center">'.$reposicion.'</td>
							</tr>';
		 }
			$contenido .= '</table>';
		echo $contenido;	
