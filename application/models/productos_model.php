...
class Productos_model extends CI_Model
{
        ...
        if($_POST['prod_id']!='')
        {
            $dataUpdate = [
                           ...
                           // 'prod_imagen' => $_FILES['prod_imagen']['name'],
                            ...
                            'prod_tipo_insumo'     => $_POST['tipo_insumo'],
               ...
               // codigo para que no se borre la imagen cada vez que haga una actualizacion 
            ...
            // VALIDAMOS SI EL PRODUCTO ESTA DENTRO DE UN KIT PARA ACTUALIZAR EL PRECIO DEL PROD KIT
            // SMITH FERNANDEZ CAHUANA 17/08/2021
              $rowkit=$this->db->select('prod_id')
                    ->from('producto_presentaciones')
                    ->where('prod_c_id',$_POST['prod_id'])
                    ->get()
                    ->row();
                if ($rowkit) {
                    // actualizamos el precio del producto kit
                     ...
                     $rsProductoPres =  $this->db->from('producto_presentaciones prp')
                         ...
                         ->where('prp.prod_id',$rowkit->prod_id)
                         ->get()
                         ->result();
                 ...
                 $prodkitupdate = array('prod_precio_publico' =>$montoTotalKit, );
                    $this->db->where('prod_id',$rowkit->prod_id)
    ...
    public function getMainList() {
        ...
        if($_POST['search']!='')
        {
           $query = "(prod.prod_nombre like '%".$_POST['search']."%' or prod.prod_codigo like '%".$_POST['search']."%' or prod_codigo_barra LIKE '%".$_POST['search']."%')";
            $select->where($query);
            $select->order_by("prod.prod_nombre");  
        }  else{
          $select->order_by("prod.prod_id desc");  
        } 
        ...
        foreach($rsProductos as $producto) {
            ...
            $producto->prod_presentacion = "";
            if($producto->prod_tipo_insumo == 2)
            $producto->prod_presentacion = "<a class='btn btn-default btn-xs btn_agregar_presentacion'
                                            data-id='{$producto->prod_id}' data-toggle='modal' data-target='#myModal'>PRODUCTO KIT</a>";

            $producto->prod_agregarItem = "<a class='btn btn-default btn-xs btn_agregarItem' data-id='{$producto->prod_id}' data-nombre='{$producto->prod_nombre}' data-precio='{$producto->prod_precio_publico}'><i class='glyphicon glyphicon-share-alt'></i></a>";
    ...
    public function selectAutocompleteprod($buscar){ 
        ...
        foreach ($result as $prod){
          ...
          $montoTotalKit = 0;
           // validamos si es producto kit para que traiga el precio
                 if ($prod->prod_tipo_insumo == 2 ) {
                  
                    $rsProductoPres =  $this->db->from('producto_presentaciones prp')
                         ->join('productos pro','pro.prod_id = prp.prod_c_id')
                         ->where('estado',ST_ACTIVO)
                         ->where('prp.prod_id',$prod->prod_id)
                         ->get()
                         ->result();
                         foreach ($rsProductoPres as $value ) {
                   $montoTotalKit =  $montoTotalKit + $value->cantidad*$value->prod_precio_publico;
                         }
                  $prod->prod_precio_publico = $montoTotalKit;
                 }
        ...
        foreach ($result as $prod){
          ...
          $producto_stock = ($stock!='')?$stock:'0';
            ...
            $data[] = array(
                ...
                "categoria" => $prod->cat_nombre,
                "prod_medida_id" => $prod->prod_medida_id,
                "prod_tipo_insumo" => $prod->prod_tipo_insumo,
                "medida" => $prod->medida_nombre
