    ...
    public function guardarGuia()
    {
      ...
      if(!is_int($_POST['cliente_id'])){
        ...
        if($_POST['guiaId'] == '')
        {
            ...
            $correlativo2 = $this->maximoConsecutivo2();
            ...
            $dataInsert['guia_numero'] = $correlativo2;
            $dataInsert['orden'] = $_POST['orden'];
            ...
            $dataInsert['destinatario_razon_social'] = $_POST['destinatario_razon_social2'];
            $dataInsert['ubigeo_partida'] = substr($_POST['ubigeo_partida'],0,6);
            $dataInsert['ubigeo_llegada'] = substr($_POST['ubigeo_llegada'],0,6);
            //$dataInsert['ubigeo_partida'] = $_POST['ubigeo_partida'];
            //$dataInsert['ubigeo_llegada'] = $_POST['ubigeo_llegada'];
            ...
            $dataInsert['destinatario_direccion_fiscal'] = $_POST['llegada_direccion2'];
          ...
          //var_dump($_POST['destinatario_razon_social']);exit;
            ...
            $dataUpdate['destinatario_razon_social'] = $_POST['destinatario_razon_social2'];
            $dataUpdate['ubigeo_partida'] = substr($_POST['ubigeo_partida'],0,6);
            $dataUpdate['ubigeo_llegada'] = substr($_POST['ubigeo_llegada'],0,6);
            //$dataUpdate['ubigeo_partida'] = $_POST['ubigeo_partida'];
            //$dataUpdate['ubigeo_llegada'] = $_POST['ubigeo_llegada'];
            ...
            $dataUpdate['destinatario_direccion_fiscal'] = $_POST['llegada_direccion2'];
                  ...
                  // VALIDACION PARA RESTAURAR PRODUCTOS KIT    
                               if ($result->prod_tipo_insumo == 2) {
                                      ...
                                      foreach ($rsProductokit as  $value) {
                                         $concepto = 'MODIFICAR GUIA - RESTAURAR / '.$result->prod_nombre;
                                         //$this->productos_model->salidaStock($value->prod_c_id,$cantidades[$i]*$value->cantidad,$concepto,$rsProducto->prod_id);
                          $this->productos_model->ingresarStock($value->prod_c_id,$item->cantidad*$value->cantidad,$concepto,$idGuia,strtoupper($_POST['serie']),$_POST['numero']);  
                                      }
                                 }
                              
                                $concepto = 'MODIFICAR GUIA - RESTAURAR';
                                $this->productos_model->ingresarStock($item->producto_id,$item->cantidad,$concepto,$idGuia,strtoupper($_POST['serie']),$_POST['numero']);                                                                                           
                } 
            }           
        }    
       ...
       //   $dataInsertDetalle = [
         //                           "producto_id"  => $item,
          ...
          $this->db->insert("guia_detalles", $dataInsertDetallee); 
              ...
              $datosGuia = $this->db->from('guias g')
                                    ->where('g.id',$idGuia)
                                    ->get()
                                    ->row();
                
          // codigo para descontar stock si es un producto kit
          if($_POST['descontar_stock']==on){ // VALIDACION PARA CUANDO SE HACE UNA GUIA CON  DOCUMENTO Y ES UN KIT
            if ($rsProducto->prod_tipo_insumo == 2) {

              $rsProductokit = $this->db->from('producto_presentaciones')
                                                ->where('prod_id',$rsProducto->prod_id)
                                                ->where('estado',ST_ACTIVO)
                                                ->get()
                                                ->result();
                              
                                      foreach ($rsProductokit as  $value) {
                                         $concepto = 'VENTA PRODUCTO-KIT/GUIA/ '.$rsProducto->prod_nombre;
                                         //var_dump($concepto);exit;
                                         $this->productos_model->salidaStock($value->prod_c_id,$cantidades[$i]*$value->cantidad,$concepto,$rsProducto->prod_id,$datosGuia->guia_serie,$datosGuia->guia_numero);
                                      }
               }
            }
...
/* FERNANDEZ CAHUANA SMITH  15/05/20222 */
    public function maximoConsecutivo2()
    {
      //obtenemos el maximo consecutivo de la columna guia_numero
      $select = $this->db->select(' MAX(CAST(guia_numero AS UNSIGNED)) numero','left') 
                              ->from("guias")
                              ->where("estado", ST_ACTIVO)
                              ->get()
                              ->row();
       $rsMayorConsecutivo = $select->numero;
        $rsMayorConsecutivo++;
        return $rsMayorConsecutivo;                              
    }
    ...
    //DEVOLVER STOCK
    public function devolverStock($idGuia){

            $rsGuia = $this->guias_model->select($idGuia);
            
        
         foreach ($rsGuia->detalles as $value) {
            // var_dump($value);exit;
            //validacion para que devuelva el stock si es un producto kit
            if ($value->prod_tipo_insumo == 2) {
               ...
               $rsProductos = $this->db->from('producto_presentaciones prp')
                                                    ...
                                                    ->where('prp.prod_id',$value->producto_id)
                                                    ->where('prp.estado',ST_ACTIVO)
                                                    ->get()
                                                    ->result();
               foreach ($rsProductos as  $value2) {
               
                 $concepto = 'CONVERSION COMPROBANTE - RESTAURAR / '.$value->descripcion;
                                         //$this->productos_model->salidaStock($value->prod_c_id,$cantidades[$i]*$value->cantidad,$concepto,$rsProducto->prod_id);
                        $this->productos_model->ingresarStock($value2->prod_c_id,$value2->cantidad*$value->cantidad,$concepto,$rsGuia->id,strtoupper($rsGuia->guia_serie),$rsGuia->guia_numero); 
                ...
                $stock = $this->productos_model->getStockProductos($value->producto_id,$this->session->userdata("almacen_id"));
            
            $nueva_cantidad = floatval($stock) + floatval($value->cantidad);
           
            $concepto = 'RESTAURAR - GUIA';
            $kardex = array(
                          'k_fecha' => date('Y-m-d'),
                          'k_almacen' => $this->session->userdata("almacen_id"),
                          'k_tipo' => 1,
                          'k_operacion_id' => $value->guia_id,
                          'k_serie' => $rsGuia->guia_serie."-".$rsGuia->guia_numero,
                          'k_concepto' => $concepto,     
                          'k_producto' => $value->producto_id,
                          'k_ecantidad' => $value->cantidad,
        ...
        $this->db->where('id',$idGuia);
        $this->db->update('guias',array('estado' => 4));
