...
class Notas_model extends CI_Model
{
    ...
    public function select($id = '', $activo = '')
    {
        ...
        $rsDetalles = $this->db->from("nota_pedido_detalle")
                               ->join('productos pr',"pr.prod_id=notapd_producto_id",'left')
    ...
    {      
        ...
        if($_POST['notaId'] == '')
        {
            ...
            $dataInsert['notap_pago_monto'] = $_POST['pago_monto'];
            ...
            $dataUpdate['notap_pago_monto'] = $_POST['pago_monto'];
            ...
            $idNota = $_POST['notaId'];       
             ...
             // traemos la data de la nota
            $datosnotas= $this->db->from('nota_pedido np')
                                ->where('np.notap_id',$idNota)
                                ->get()
                                ->row();
                  ...
                  if($result->prod_tipo == 1){
                   ...
                   if($result->prod_tipo_insumo == 2){//PRODUCTO KIT
                        ...
                        foreach ($rsProductos as  $value) {
                                ...
                                $this->productos_model->ingresarStock($value->prod_c_id,$item->notapd_cantidad*$value->cantidad,$concepto,$result->prod_id,'NP',$datosnotas->notap_correlativo);
                                     }
              } else {
           ...
           // traemos la data de la nota
            $datosnotas2= $this->db->from('nota_pedido np')
                                ->where('np.notap_id',$idNota)
                                ->get()
                                ->row();   
                  ...
                  //PRODUCTO KIT - ALEXANDER FERNANDEZ DE LA CRUZ 02-12-2020
                   if($result->prod_tipo_insumo == 2){//PRODUCTO KIT                    
                        $rsProductos = $this->db->from('producto_presentaciones')
                                                ...
                                                ->result();                                                
                        /*foreach ($rsProductos as  $value) {
                                $concepto = 'NOTA VENTA PRODUCTO-KIT/ '.$result->prod_nombre;
                                $this->productos_model->salidaStock($value->prod_c_id,$_POST['cantidad'][$i]*$value->cantidad,$concepto,$result->prod_id);
                                    }*/
                        ...
                        foreach ($rsProductos as  $value) {
                            if(!in_array($value->id,$deleteKit)){
                                $concepto = 'NOTA VENTA PRODUCTO-KIT/ '.$result->prod_nombre;
                                $this->productos_model->salidaStock($value->prod_c_id,$cantidad[$i]*$value->cantidad,$concepto,$result->prod_id,'NP',$datosnotas2->notap_correlativo);                                
                        }} 
              } else {
         ...
         foreach ($rsNota->detalles as $value) {
            ...
            if ($value->prod_tipo_insumo == 2) {
               ...
               $rsProductos = $this->db->from('producto_presentaciones prp')
                                                    ...
                                                    ->where('prp.prod_id',$value->notapd_producto_id)
             ...
             //var_dump($rsProductos);exit;
               foreach ($rsProductos as  $value2) {
                 ...
                 $concepto = 'CONVERSION COMPROBANTE - RESTAURAR / '.$value->notapd_descripcion;
                                         //$this->productos_model->salidaStock($value->prod_c_id,$cantidades[$i]*$value->cantidad,$concepto,$rsProducto->prod_id);
                        $this->productos_model->ingresarStock($value2->prod_c_id,$value2->cantidad*$value->notapd_cantidad,$concepto,$rsGuia->id,'NP',$rsNota->notap_correlativo); 
