...
class Ingresos_model extends CI_Model
{
    ...
    public function guardarIngreso()
    {
         if($_POST['ing_id'] != '')
        {
            ...
            $dataUpdate = [
                            "k_almacen" => $_POST['almacen']
                          ];            

            $this->db->where("k_operacion_id", $_POST['ing_id']);
            $this->db->where("k_tipo", 5);            
            $this->db->update("kardex", $dataUpdate);
    ...
    public function guardarProductoIngreso()
    {
      ...
      $concepto = 'MOVIMIENTO DE ALMACEN INGRESO';
      $this->productos_model->ingresarStock($_POST['producto'],$_POST['cantidad'],$concepto,$ingresoId);
      return true;
    }
    ...
    public function eliminarDetalleIngreso($idDetalle)
    {
        ...
        $concepto = 'MOVIMIENTO DE ALMACEN INGRESO - RESTAURAR';
        $this->productos_model->salidaStock($rsDetalle->ingd_producto_id,$rsDetalle->ingd_cantidad,$concepto,$rsDetalle->ingd_ingreso_id);                                              
