...
<script>
    ...
    $("#grid").kendoGrid({
      ...
      columns: [
                    ...
                    {field:'prod_nombre',title:'NOMBRE/DESCRIPCIÓN',width:'230px'},
                    {field:'cat_nombre',title:'CATEGORIA',width:'50px'},
                    {field:'medida_nombre',title:'UNIDAD/MEDIDA',width:'50px'},
                    {field:'prod_precio_publico',title:'PRECIO V',width:'40px'},
                    {field:'prod_precio_compra',title:'PRECIO C',width:'40px'},
                    //{field:'prod_cantidad_minima',title:'CANTIDAD MINIMA',width:'160px'},
                    {field:'prod_presentacion',title:'PRODUDCTO KIT',width:'55px',template:"#= prod_presentacion #"},
                    {field:'prod_stock', title:'STOCK',width:'30px'},
                    {field:'prod_codigo_barra', title:'CODIGO BARRA',width:'50px'},
                    //{field:'alm_nombre', title:'ALMACÉN',width:'100px'},
                    {field:'prod_editar', title:'&nbsp;',width:'25px',template:"#= prod_editar #"},
                    {field:'prod_eliminar', title:'&nbsp;',width:'25px',template:"#= prod_eliminar #"},
        ],
        dataBound:function(e){

            //nueva presentacion
            $(".btn_agregar_presentacion").click(function(e){                
                var idProducto = $(this).data('id');
                var idView = 0;
                $("#myModal").load('<?PHP echo base_url()?>index.php/productos/agregarPresentacion/'+idProducto+'/'+idView,{});
            });
            //modificar producto
...
//ELIMINAR PRESENTACION
    $(document).on("click",".btn_eliminar_presentacion",function(){
        ...
        $.confirm({
            ...
            buttons:{
                confirm:{
                    ...
                    action: function(){
                        $.ajax({
                        url :'<?= base_url();?>index.php/productos/eliminarPresentacion/'+ $idPresentacion,
                        dataType: 'JSON',           
                        method: 'GET',
                        success: function(response){                        
                            ...
                            if(response.status == STATUS_OK){
                                ...
                                toast('succes',1500,'producto eliminado');
                                selectPresentacion();
                              // para que actualize el precio kit cada vez elimina
                               dataSource.read();   
                            }
                            if(response.status == STATUS_FAIL){
                                toast('error',1500,'No se puede eliminar producto');
                            }                       
                        }
                    })
                    }
