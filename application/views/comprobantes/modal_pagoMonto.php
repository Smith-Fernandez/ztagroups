...
<script type="text/javascript">
        ...
        function agregarFilaPagoMonto(){ 
            var rowMontoPago = $(".montoPago").toArray().length;                        
              ...
              var  fila = '<div class="panel panel-default cont-item montoPago">';                 
                 ...
                 fila += '<label class="tipo_pagoMonto1">Tipo Pago '+
                ...
                fila += '<div class="col-xs-12 col-md-6 col-lg-6 importe_pagoMontopadre">'
                fila += '<label>Monto'+
                ...
                fila += '<div class="col-xs-12 col-md-6 col-lg-6 pago_neto" style="display:none;" >'
                fila += '<label>Monto Neto'+
                         '<input type="text" id="importe_pagamontoneto" name="importe_pagamontoneto[]" class="form-control importe_pagamontoneto" readonly></label></div>';           
                fila += '<div class="col-xs-12 col-md-6 col-lg-6">'                     
                fila += '<label>Observacion'+
                        '<input type="text" id="observacion_pagoMonto" name="observacion_pagoMonto[]" required="" class="form-control observacion_pagoMonto"></label></div>';
                         // codigo para que aparezca el pago por cuota
                fila += '<div class="col-xs-12 col-md-6 col-lg-6 pago_cuota" style="display:none;">'                     
                fila += '<label>fecha pago (cuota)'+
                        '<input type="text"  name="cuota_pagoMonto[]" required="" class="form-control cuota_pagoMonto"></label></div>';
    ...
    function guardarComprobante(){        
        ...
        url     = (action == 1) ? '<?PHP echo base_url()?>index.php/comprobantes/modificar_comprobante':'<?PHP echo base_url()?>index.php/comprobantes/guardar_comprobante';
        mensaje = (action == 1) ? 'Modificación existosa':'Comprobante registrado';
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                {                    
                    if($("#auto").val() == 1 && action != 1) {                        
                        send_xml(response.cpe_id);
                    }else{     
                         url =  (action != 1) ? '<?PHP echo base_url()?>index.php/comprobantes/index/'+response.cpe_id : '<?PHP echo base_url()?>index.php/comprobantes';
                         toast("success", 1500, mensaje);
                         setTimeout(function() { 
                           location.href = url;
                         }, 2000);
                    }       
   ...
   // codigo para el cuando es credito
   $(document).on("change",".tipo_pagoMonto",function(e){
         ...
         { 
            ...
            var rowMontoPago = $(".montoPago").toArray().length; 
             if ($('#retencion').is(':checked')) {
                    ...
                    total = _item.find('.importe_pagoMontopadre').find('#importe_pagoMonto').val();
                    total_retencion = parseFloat(total * (porcentaje_de_retencion/100)).toFixed(2);

                     importe_pagoMonto =  parseFloat(total - total_retencion).toFixed(2);
                     console.log(importe_pagoMonto);
                     //alert(importe_pagoMonto);
                   // $('#importe_pagoMonto').val(importe_pagoMonto);
                    //$('#importe_pagamontoneto').val(importe_pagoMonto);
                    _item.find('.pago_neto').find('#importe_pagamontoneto').val(importe_pagoMonto);
                    //$('#pago').val(importe_pagoMonto);
                       _item.find('.pago_neto').css('display','block');
                }
             /* si es una detraccion */
            ...
            if ($('#detraccion').is(':checked')) {
                ...
                var porcentaje_de_detraccion = $('#porcentaje_de_detraccion').val();
                    ...
                    total_detraccion = total * (porcentaje_de_detraccion/100);
                     //alert(importe_pagoMonto);
                     importe_pagoMonto = parseFloat(total - total_detraccion).toFixed(2);
            ...
            //$('.pago_cuota').show();
             console.log(_item[0]);
              _item.find('.pago_cuota').css('display','block');
         }
        else
         {
            //limpiamos el valor si el cliente a marcado anteriormente credito y despues efectivo
              _item.find('.cuota_pagoMonto').val('');
              _item.find('.pago_cuota').css('display','none');
                _item.find('.pago_neto').css('display','none');

                 /* dejamos el monto como estaba */
                    total = $('#total_a_pagar').val();
              
                     //alert(importe_pagoMonto);
                     importe_pagoMonto = total;
   ...
   // para aplicarle el datepicker al momento que se le da click

    $(document).on('focus', '.cuota_pagoMonto',function(){
            $(this).datepicker({
                todayHighlight:true,
                format:'yyyy-mm-dd',
                autoclose:true
...
//asignando datetimepicker con delegate
/* $( document ).ready(function() {
    //$('.cuota_pagoMonto').datepicker();

     $('body').delegate('.cuota_pagoMonto', 'click', function() {
       // alert("dd");
