                        ...
                        <div class="col-md-12 form-inline col-lg-12">                            
                            ...
                            <?PHP foreach ($tipo_documentos as $value) { ?>   
                               <?php if($value['id']!=11) {?>  
                                    ...
                                    <?php }else{ ?>
                                         <option value = "<?PHP echo $value['id'];?>" <?php echo ($value['id']==1)?"selected":"";?>><?PHP echo $value['tipo_documento']?></option>
                                    <?php } ?>  
                            ...
                            <div class="row" id="valida">
                                <div class="col-xs-12 col-md-12 col-lg-12">
                                    <table id="tabla" class="table" style="display:none" border="0">
                                        <thead>
                                            <tr>                                                
                                                <th colspan="2" class="col-sm-3">Descripcion</th>
           ...
           <!-- MUESTRA DETRACCION , FACTURA O BOLETA -->
            ...
            <div class="panel panel-body">
                ...
                <label for="detraccion" class="label-primary"></label>
                 ...
                 <div class="col-xs-3 col-md-3 col-lg-3">
                  ...
                  <?PHP foreach ($tipo_detracciones as $value) { ?>                          
                      <option value = "<?PHP echo $value->id;?>"><?PHP echo $value->codigo.' '.$value->tipo_detraccion?></option>
                  <?PHP }?>
                  </select>    
                </div>                
                <div class="col-xs-3 col-md-3 col-lg-3">
                ...
                <div class="col-xs-2 col-md-2 col-lg-2">
                    ...
                    <input type="text" class="form-control" name="porcentaje_de_detraccion" id="porcentaje_de_detraccion" value="10">
                </div>                
                <div class="col-xs-2 col-md-2 col-lg-2">
                    <label class="control-label">Total de Detraccion:</label>
                    <input type="text" class="form-control" name="total_detraccion" id="total_detraccion">
                </div>                
                </div>
            </div>
            ...
            <div class="panel panel-body">
                ...
                <div class="col-xs-3 col-md-3 col-lg-3">
                    ...
                    <input type="text" class="form-control" name="total_retencion" id="total_retencion">
                </div>                
                </div>
            </div>
            ...
            <!--<?php if ($configuracion->notas): ?>                -->
                <div class="panel panel-info" id="panel_otros">
                    ...
                    <div class="panel-body">
                        <textarea name="notas" id="notas" rows="3" cols="100"></textarea>
                    </div>
                </div>            
            <!--<?php endif ?>-->
            <div class="panel panel-info" id="panel_otros">
...
<script type="text/javascript"> 
    ...
    function consulta_sunat(){
        ...
        if(num!=''){
        if(num.length == 8){//DNI
            $.getJSON('http://mundosoftperu.com/reniec/consulta_reniec.php',{dni:num})
             .done(function(json){                
            ...
            $.getJSON('http://mundosoftperu.com/sunat/sunat/consulta.php',{nruc:num})
             .done(function(json){
          ...
          .done(function(json){
             ...
             if(json.det.length>0){  
               $.each(json.det,function(index,value){
                  ...
                  fila += '<td style="border:0;"><input type="text" class="form-control" readonly  value="'+ value.notapd_medida +'"><input type="hidden" class="form-control" readonly name="medida[]" id="medida" value="'+ value.notapd_medida_id +'"></td>';
               ...
               $("#direccion").val(json.doc.destinatario_direccion_fiscal);
    ...
    $('body').delegate('.descripcion-item', 'keydown', function() {
        $('.descripcion-item').autocomplete({
            ...
            select : function (event,ui){                
                ...
                var data_kit  = (ui.item.prod_tipo_insumo == 2) ? '<label class="col-sm-1 control-label glyphicon glyphicon-triangle-top btn_agregar_presentacion" data-id="'+ui.item.id+'" data-view="1" data-toggle="modal" data-target="#myModal"></label>': '<label class="col-sm-1 control-label glyphicon glyphicon-triangle-top"></label>';

                _item.find('#data_item').html(data_item);
                _item.find('#data_kit').html(data_kit);
     ...
     //EVENTO COMBOBOX TIPO DE CAMBIO
       ...
       //RECIBIENDO DATOS COMPROBANTE A FACTURAR
        ...
        function agregarFila(){    
            var fila = '<tr class="cont-item" >';                                              
                fila += '<td colspan="2" class="col-sm-4" style="border:0;"><div id="data_kit"><label class="col-sm-1 control-label glyphicon glyphicon-triangle-top"></label></div>'+
                        '<div class="col-sm-11"><input class="form-control descripcion-item" rows="2" id="descripcion" name="descripcion[]" required="">'+
                        '<div id="data_item"><input type="hidden" name="item_id[]" id="item_id"></div></div><input type="hidden" name="productoKit[]" class="productoKit"></div></td>';
    ...
    //CARGAR MODAL CONFIGURAR PRODUCTO KIT    
    $(document).on("click",".btn_agregar_presentacion",function(){        
        var idProducto = $(this).data('id');        
        var idView = $(this).data('view');
        var productokit = $(this).closest('.cont-item').find('.productoKit').val();        
        $("#myModal").load('<?PHP echo base_url()?>index.php/productos/agregarPresentacion/'+idProducto+'/'+idView+'/'+productokit,{});
    });
    ...
    $(document).keydown(function(e) {       
        ...
        if(code=='13' && barcode == '')// Enter key hit
        {
            e.preventDefault();
            var tabla = $('#tabla > tbody > tr');
