...
//
    ...
    public function guardarNota()
    {
        ...
        foreach ($idproduc as $item) {                    
                ...
                //CONSULTA DE STOCK PARA PRODUCTO KIT -- ALEXANDER FERNANDEZ
                ...
                foreach ($rsProductos as  $value) {
                                ...
                                if($value->prod_tipo == 1){
                                    if($_POST['notaId'] == ''){
                                        if(($cantidad[$i]*$value->cantidad == 0) OR ($cantidad[$i]*$value->cantidad >$prod_stock)){
                                        sendJsonData(['status'=>STATUS_FAIL, 'msg'=>'No hay stock disponible de '.$value->prod_nombre]);
                                        exit();
                                        }
                                    }else{

                                        $item_producto = $this->db->from('nota_pedido_detalle')
                                                                  ->where('notapd_producto_id',$idproduc[$i])                    
                                                                  ->where('notapd_notap_id',$_POST['notaId'])
                                                                  ->get()
                                                                  ->row();

                                        if(($cantidad[$i]*$value->cantidad == 0) OR ($cantidad[$i]*$value->cantidad > ($prod_stock + ($item_producto->notapd_cantidad*$value->cantidad)))){
                                        sendJsonData(['status'=>STATUS_FAIL, 'msg'=>'No hay stock disponible de '.$value->prod_nombre]);
                                        exit();
                                        }                                                                                                          
                                    }
    ...
    public function decargarPdf($idNota)
    {
        ...
        /*formateamos fecha*/
        $rsNota->notap_fecha = (new DateTime($rsNota->notap_fecha))->format("d/m/Y h:i:s A");                   
        $rsDetalles =  $this->db->from("nota_pedido_detalle as f")
                                ->join('productos as p','p.prod_id=f.notapd_producto_id','left') 
                                ->join('medida as m','m.medida_id=f.notapd_unidad_id','left')
                                ->where("f.notapd_notap_id", $idNota)
                                ->get()
                                ->result();        
              //var_dump($rsDetalles);exit;             
        ...
        $this->pdf->setPaper('A4', 'portrait');
        $this->pdf->render();
        $this->pdf->stream("N.Venta.NP-$rsNota->notap_correlativo.pdf",
            array("Attachment"=>0)
        );
    }
            ...
            foreach ($adp as $a) {
                $result = $this->db->from('productos')
                  ...
                  if($result->prod_tipo==1){
                    ...
                    //PRODUCTO KIT - ALEXANDER FERNANDEZ DE LA CRUZ 02-12-2020
                   if($result->prod_tipo_insumo == 2){//PRODUCTO KIT
                        ...
                        foreach ($rsProductos as  $value) {
                            //var_dump($result->prod_nombre);exit;
                                $concepto = 'NOTA VENTA PRODUCTO-KIT RESTAURAR/ '.$result->prod_nombre;
                                $this->productos_model->ingresarStock($value->prod_c_id,$a->notapd_cantidad*$value->cantidad,$concepto,$result->prod_id);
                                     }
                                }
    ...
    public function exportarExcel()
    {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('P1', "TRANSPORTISTA");
        $objPHPExcel->getActiveSheet()->setCellValue('Q1', "PLACA");        
        $objPHPExcel->getActiveSheet()->setCellValue('R1', "ALMACEN");
        ...
        foreach ($resultComprobantes as $value) {           
            ...
            $objPHPExcel->getActiveSheet()
                        ...
                        ->setCellValue('O'.$i, $rsUtilidad->totalVenta - $rsUtilidad->totalCosto)
                        ->setCellValue('P'.$i, $rsTransportista->transp_nombre)
                        ->setCellValue('Q'.$i, $rsCliente->placa)
                        ->setCellValue('R'.$i, $this->session->userdata('almacen_nom'))
                        ->setCellValue('S'.$i, $value->notap_observaciones);  
     ...
     public function exportarExcel_rd()
    {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('H1', "TIPO PAGO");
        $objPHPExcel->getActiveSheet()->setCellValue('I1', "UTILIDAD");
        $objPHPExcel->getActiveSheet()->setCellValue('J1', "TRANSPORTISTA");
        $objPHPExcel->getActiveSheet()->setCellValue('K1', "PLACA");        
        $objPHPExcel->getActiveSheet()->setCellValue('L1', "ALMACEN");
        $objPHPExcel->getActiveSheet()->setCellValue('M1', "OBSERVACION");
        ...
        foreach ($resultComprobantes as $value) {           
            ...
            $objPHPExcel->getActiveSheet()
                        ...
                        ->setCellValue('H'.$i, $rsPago)
                        ->setCellValue('I'.$i, $rsUtilidad->totalVenta - $rsUtilidad->totalCosto)
                        ->setCellValue('J'.$i, $rsTransportista->transp_nombre)
                        ->setCellValue('K'.$i, $rsCliente->placa)
                        ->setCellValue('L'.$i, $this->session->userdata('almacen_nom'))
                        ->setCellValue('M'.$i, $value->notap_observaciones);  

            $suma_total += $value->notap_total;
            $i++;
        }

         $objPHPExcel->getActiveSheet()->getStyle('F' . ($i +1 ))->getFont()->setBold(true);
         $objPHPExcel->getActiveSheet()->setCellValue('F' . ($i +1 ), 'MONTO TOTAL');
         $objPHPExcel->getActiveSheet()->setCellValue('G' . ($i +1 ), $suma_total);
