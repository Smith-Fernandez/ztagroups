...
class Productos extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('producto_presentaciones_model');
        $this->load->model('categoria_model');
        $this->load->model('medida_model');
        $this->load->model('tipo_items_model');
        $this->load->model('tipo_insumos_model');
    ...
    public function crear()
    {
        ...
        $data['tipo_insumos'] =$this->tipo_insumos_model->select();
    ...
    public function guardarSubidaProductos() {     
      ...
      public function ExportarExcel($nombre='') {
       if($this->uri->segment(3)!='0') {
            $cadena =str_replace('%20', ' ',$this->uri->segment(3));
            $this->db->like('prod_nombre',$cadena);
        }
   ...
   public function selectPrecioCodBarra() {
    ...
    //ALEXANDER FERNANDEZ DE LA CRUZ - 30-11-2020
    public function agregarPresentacion($idProducto,$idView,$productoKit=''){        
        $data['producto'] = $this->productos_model->select($idProducto);
        $data['idView'] = $idView;
        $data['productoKit'] = $productoKit;
        //var_dump($productoKit);exit;
        echo $this->load->view('productos/modal_agregarPresentacion',$data);
    }              
    ...
    public function selectPresentacion(){
        ...
        $idView = $_POST['idView'];
        $deleteKit= $_POST['deleteKit'];
        $deleteKit = explode(",", $deleteKit);        


        $montoTotalKit = 0;
        //TRAE PRESENTACIONES
        $rsPresentaciones = $this->producto_presentaciones_model->selectPresentacion($idProducto,$idView);
        echo '<table class="table table-striped">';
        foreach ($rsPresentaciones as $value) {            
            if(in_array($value->id,$deleteKit)){
                $cssEliminado = "class ='eliminado' style='display:none'";
                $value->cantidad = 0;
            } else{
               $cssEliminado = " "; 
            }                           
            ...
            echo '<tr '.$cssEliminado.'><td>'.$value->prod_nombre.'</td><td>'.$value->cantidad.'</td><td>'.$value->eliminar.'</td></tr>';            
        }
        echo '<input type="hidden" id="totalProductoKit" value="'.$montoTotalKit.'"></table>';
    } 

    public function guardarPresentacion(){
        $idProducto = $_POST['idProducto'];
        $this->producto_presentaciones_model->guardar();        
    }

    public function eliminarPresentacion($idPresentacion){
        //echo $idPresentacion;exit;
        $result = $this->producto_presentaciones_model->eliminar($idPresentacion);
