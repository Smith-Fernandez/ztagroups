...
class Comprobantes extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('tipo_detracciones_model');
    ...
    public function nuevo($factura_id = '', $valida = 0) {
        ...
        $data['tipo_detracciones'] = $this->tipo_detracciones_model->select();
    ...
    public function modificar() {                
       ...
       // print_r($data['comprobante']);exit();
        ...
        $data['tipo_ndebitos'] = $this->tipo_ndebitos_model->select('', '', '', 0);
        $data['elemento_adicionales'] = $this->elemento_adicionales_model->select('', '', 'activo');
        /*mandamos para que liste tambien las detracciones*/
        $data['tipo_detracciones'] = $this->tipo_detracciones_model->select();
        $data['monedas'] = $this->monedas_model->select();
        $data['empresas'] = $this->empresas_model->select();
        $data['comp_adjuntos'] = $this->comprobantes_model->select('', '', '', '', '', '', '', '0');
    ...
    public function pdfGeneraComprobanteOffLine($comprobante_id = '', $vista = '') {
        ...
        $rsComprobante = $query->row();
        /*obtenemos el detalle del documento*/
        $this->db->select('i.*,p.*,m.medida_codigo_unidad,m.medida_nombre')
                 ->from("items as i")
        ...
        $rsPagoMonto =  $this->db->from('comprobante_pagos cmp')
                                ...
                                ->where('cmp.estado',ST_ACTIVO)
                                ->get()
                                ->result();
    //Monto total para la forma de pago SMITH FERNANDEZ 22/05/2022
        $total_pago = 0;
          foreach ($rsPagoMonto as  $value) {
                    if ($rsComprobante->retencion == 1 || $rsComprobante->detraccion == 1 ) {
                        if ($value->tipo_pago_id == 2) {
                            $total_pago += $value->monto_neto;
                        }
                    }else{
                      $total_pago +=  $value->monto;
                    }
           }                     
      
                                
//CALCULO PARA LA CANTIDAD DE DIAS AL CREDITO
        $date1 = new DateTime($rsComprobante->fecha_de_emision);
        $date2 = new DateTime($rsPagoMonto[0]->fecha_cuota);
        $diff= $date1->diff($date2);
        $rsComprobante->ndias = $diff->days;
       // $rsPagoMonto->fecha_cuota = (new DateTime($rsPagoMonto->fecha_cuota))->format("d/m/Y h:i:s A");
        //var_dump($rsPagoMonto);exit;
        $rsComprobante->fecha_de_emision = (new DateTime($rsComprobante->fecha_de_emision))->format("d/m/Y h:i:s A");
        $rsComprobante->fecha_de_vencimiento = ($rsComprobante->fecha_de_vencimiento!='')?(new DateTime($rsComprobante->fecha_de_vencimiento))->format("d/m/Y"):'';
        /*documento relacionado*/
        $rsRelacionado = $this->db->from("comprobantes")
                                  ->where("id", $rsComprobante->com_adjunto_id)
...
//        echo $obj['contenido'];
    ...
    public function buscador_itemC() {
     ...
     public function guardar_comprobante() {
        ...
        /* para validar q no tenga retencion detraccion a la vez */
        $detraccion = isset($_POST['detraccion']) ? 1 : 0;
        $retencion  = isset($_POST['retencion']) ? 1 : 0;

       
        if ( $detraccion == 1 and $retencion == 1) {
            
               sendJsonData(['status'=>STATUS_FAIL, 'msg'=>"el comprobante no puede tener retencion y detraccion a la vez"]);
                  exit();
        }

        //para validar que ingrese fecha de pago
        $tipo_pago = $_POST['tipo_pagoMonto'];
        $fecha_cuota = $_POST['cuota_pagoMonto'];
        $z=0;
        $x=0;
        ...
        foreach ($tipo_pago as  $value) {
         if ($tipo_pago[$z] == 2) {             
           if ($fecha_cuota[$z] =='') {
                  sendJsonData(['status'=>STATUS_FAIL, 'msg'=>"Ingrese fecha de pago"]);
                  exit();
            }
           }
          $z++;
        ...
        $productoKit = $_POST['productoKit'];


         /*validacion cuando falla el  calculo del javascript monto del javascript*/
            $total_acumulado = 0;
         /*consultamos si el igv esta activado*/
         $igv = $this->db->from('comprobantes_ventas')
                     ->get()
                     ->result();
         $igv_valor = $this->db->from('igv')
                       ->get()
                       ->result();
            $a = 0;
            foreach ($idproduc as $value) {
                  /*acumulamos cantidad por precio*/
                  $total_acumulado += round($cantidad[$a]*$importe[$a],2);
             $a++;
              } 
              /*validamos si el igv esta incluido*/
              if ($igv[0]->pu_igv != 1) {
                  $total_acumulado = round($total_acumulado + ($total_acumulado * $igv_valor[0]->valor),2);
              } 

              if ($total_acumulado != $_POST['total_a_pagar']) {
                
                sendJsonData(['status'=>STATUS_FAIL, 'msg'=>'Error en calculo verificar CANTIDAD * PRECIO UNITARIO']);
                    exit(); 
              }
        ...
        if(!$montoPago)
        {
            ...
            exit();
                   ...
                   // DEVUELVO  STOCK SI ES UNA GUIA
                 if($_POST['guia_id'] != ''){
                     $this->guias_model->devolverStock($_POST['guia_id']);           
                                         }
                     //DEVUELVO EL STOCK
                if($_POST['notap_id'] != ''){
                ...
                foreach ($idproduc as $item) {
                   if($importe[$i] != 0 and $importe[$i] != ''){
                      if($idproduc[$i] != 0){
                        ...
                        //CONSULTA DE STOCK PARA PRODUCTO KIT
                        if($dato_prod->prod_tipo_insumo == 2){//PRODUCTO KIT
                            $rsProductos = $this->db->from('producto_presentaciones prp')
                                                    ->join('productos pro','prp.prod_c_id =  pro.prod_id')
                                                    ->where('prp.prod_id',$idproduc[$i])
                                                    ->where('prp.estado',ST_ACTIVO)
                                                    ->get()
                                                    ->result();     

                            $productoKit = $productoKit[$i];
                            $deleteKit = explode(",", $productoKit);
                            foreach ($rsProductos as  $value) {
                                if(!in_array($value->id,$deleteKit)){
                                $prod_stock = $this->productos_model->getStockProductos($value->prod_c_id,$value->prod_almacen_id);                                 
                                if($value->prod_tipo == 1){
                                    if(($cantidad[$i]*$value->cantidad == 0) OR ($cantidad[$i]*$value->cantidad >$prod_stock)){
                                        sendJsonData(['status'=>STATUS_FAIL, 'msg'=>'No hay stock disponible de '.$prod_stock.'/'.$value->prod_nombre]);
                                        exit();
                                    }
                                }                                
                            }}    
        ...
        if ($_POST['tipo_documento'] <= 3) {
            if ($_POST['tipo_documento'] == 1) {// facturas
              /*  $detraccion = isset($_POST['detraccion']) ? 1 : 0;
                ...
                $retencion  = isset($_POST['retencion']) ? 1 : 0;
                $comprobante = array_merge($comprobante, array('detraccion' => $detraccion));
                $comprobante = array_merge($comprobante, array('retencion' => $retencion));
                if ($this->input->post('tipo_detraccion') != '')
                    $comprobante = array_merge($comprobante, array('tipo_detraccion_id' => $this->input->post('tipo_detraccion')));
                if ($this->input->post('porcentaje_de_detraccion') != '')
                    $comprobante = array_merge($comprobante, array('porcentaje_de_detraccion' => $this->input->post('porcentaje_de_detraccion')));
                if ($this->input->post('porcentaje_de_retencion') != '')
                    $comprobante = array_merge($comprobante, array('porcentaje_de_retencion' => $this->input->post('porcentaje_de_retencion')));
                if ($this->input->post('numero_cuenta_detraccion') != '')
                    $comprobante = array_merge($comprobante, array('numero_cuenta_detraccion' => $this->input->post('numero_cuenta_detraccion')));                
                if ($this->input->post('total_detraccion') != ''){
                ...
                if ($this->input->post('total_retencion') != ''){
                    $montoTotalRetencion = $this->input->post('total_retencion');
                    if ($_POST['moneda_id'] > 1) {                    
                        $tipoCambio = $this->tipoCambioFecha($_POST['moneda_id'], $fecha_de_emision);
                        $montoTotalRetencion = $this->input->post('total_retencion') * $tipoCambio;
                    }
                    $comprobante = array_merge($comprobante, array('total_retencion' => $montoTotalRetencion));
                }
            }
        } else {
        ...
        foreach ($idproduc as $item) {
            ...
            $dataDetalleInsert["producto_id"] = $idproduc[$i];                
             ...
             $comprobantedatos = $this->db->from('comprobantes com')
                                                ->where('com.id',$comprobante_id)
                                                ->get()
                                                ->row();
            ...
            if($result->prod_tipo == 1){
                if($result->prod_tipo_insumo == 2){                    
                        $rsProductos = $this->db->from('producto_presentaciones')
                                                ->where('prod_id',$result->prod_id)
                        ...
                        foreach ($rsProductos as  $value) {
                            if(!in_array($value->id,$deleteKit)){
                                $concepto = 'VENTA PRODUCTO-KIT/ '.$result->prod_nombre;
                                $this->productos_model->salidaStock($value->prod_c_id,$cantidad[$i]*$value->cantidad,$concepto,$result->prod_id,$comprobantedatos->serie,$comprobantedatos->numero);                                
                        }}    
                } else {
        ...
        //CAMABIO PARA LA RETENCION Y LA DETRACCION
        $monto_neto = $_POST['importe_pagamontoneto'];   
        // CAMBIO PARA LA FECHA DE PAGO AL CREDITO  
        $cuota_pagoMonto = $_POST['cuota_pagoMonto']; 
        $minutos = new DateTime();
        $pagoMonto = array();
        $i=0;
        foreach ($tipo_pagoMonto as $value) {
                // validamos para que cuando sea un tipo de pago en efectivo no registre ninguna fecha
        if ($cuota_pagoMonto[$i]!= "") {
           $fecha_cuota= new DateTime($cuota_pagoMonto[$i]);
           $fecha_cuota = $fecha_cuota->format('Y-m-d')." ".$minutos->format('H:i:s');
         }else{
          $fecha_cuota= NULL;
         }
            ...
            $pagoMonto['fecha_cuota'] = $fecha_cuota;
            $pagoMonto['monto_neto'] = $monto_neto[$i];
    ...
    public function modificar_comprobante() {  
     ...
     /*validacion cuando falla el  calculo del javascript monto del javascript*/
        ...
        foreach($rsDetalles as $item) {
         ...
         $result = $this->db->from('productos')
                ...
                if($result->prod_tipo==1){
                   ...
                   // VALIDACION PARA RESTAURAR PRODUCTOS KIT    
                               if ($result->prod_tipo_insumo == 2) {

                                      $rsProductokit = $this->db->from('producto_presentaciones')
                                                ->where('prod_id',$result->prod_id)
                                                ->where('estado',ST_ACTIVO)
                                                ->get()
                                                ->result();
                                         
                                      foreach ($rsProductokit as  $value) {
                                         $concepto = 'MODIFICAR COMPROBANTE - RESTAURAR / '.$result->prod_nombre;
                                         //$this->productos_model->salidaStock($value->prod_c_id,$cantidades[$i]*$value->cantidad,$concepto,$rsProducto->prod_id);
                          $this->productos_model->ingresarStock($value->prod_c_id,$item->cantidad*$value->cantidad,$concepto,$comprobante_id,strtoupper($_POST['serie']),$_POST['numero']);  
                                      }
                                 }
        ...
        {
           //INSERTAR COMPROBANTES PAGOS 10-10-2020
           //DELETE COMPROBANTE_PAGOS            
           $this->db->where("comprobante_id", $comprobante_id);
           $this->db->delete('comprobante_pagos');

            $tipo_pagoMonto = $_POST['tipo_pagoMonto'];
            $importe_pagoMonto  = $_POST['importe_pagoMonto'];
            $observacion_pagoMonto = $_POST['observacion_pagoMonto'];
             //CAMBIO PARA LAS RETENCION Y LA DETRACCION
            $monto_neto = $_POST['importe_pagamontoneto'];
            // CAMBIO PARA LA FECHA DE PAGO AL CREDITO  
            $cuota_pagoMonto = $_POST['cuota_pagoMonto']; 
            $minutos = new DateTime();
            ...
            foreach ($tipo_pagoMonto as $value) {
                ...
                $pagoMonto['observaciones'] = $observacion_pagoMonto[$i];
                $pagoMonto['fecha_cuota'] = $fecha_cuota;
                //PARA LA RETENCION O LA DETRACCION
                $pagoMonto['monto_neto'] = $monto_neto[$i];
...
//            '',
    ...
    public function exportarExcel()
    {
        ...
        if($this->uri->segment(12)!='null')
        {
            $this->db->where('com.tipo_documento_id', $this->uri->segment(12));
        }       
        ...
        {
            $this->db->where('com.serie', $this->uri->segment(6));
        }         
        if($this->uri->segment(7)!='null')
        {
            $this->db->where('com.numero', $this->uri->segment(7));
        } 
        ...
        $resultComprobantes = $this->db->from("comprobantes com")                                                                           
                                      ...
                                      // var_dump($resultComprobantes);exit;
        ...
        foreach($resultComprobantes as $index => $item)
        {
            ...
            } else if($item->estado_sunat == 2){
                 $estado = 'NO ENVIADO SUNAT';
            }
    ...
    public function exportarExcel_rd()
    {
        ...
        foreach($resultComprobantes as $index => $item)
        {
            ...
            $objPHPExcel->getActiveSheet()                        
                        ...
                        ->setCellValue('K' . ($i), $item->precio_base)
                        ->setCellValue('L' . ($i), $item->cantidad)                                                                                    
                        ->setCellValue('M' . ($i), $item->total_a_pagar)                                                
                        ->setCellValue('N' . ($i), $rsPago)
                        ->setCellValue('O' . ($i), $rsEmpleado->nombre.' '.$rsEmpleado->apellido_paterno)
                        ->setCellValue('P' . ($i), $rsUtilidad->totalVenta - $rsUtilidad->totalCosto)
                        ->setCellValue('Q' . ($i), $rsTransportista->transp_nombre)
    ...
    public function buscar_guia(){
        ...
        if($res->numero_factura!=''){
            ...
            //para que traiga datos del cliente que no esta eliminado
            $this->db->where('eliminado_cliente',0);
            $cliente = $this->db->get('clientes')->row();

            $this->db->from('guia_detalles as g');
            $this->db->join('productos as p','p.prod_id=g.producto_id','left');
            $this->db->join('medida as m','m.medida_id=g.medida_id','left');
            ...
            foreach ($detalles as $d) {
               ...
               $objeto->notapd_medida_id = $d->medida_id;
    ...
    public function init_dashboard(){
       ...
       // $response['a'] = $com1->total_a_pagar+$com3->notap_total-$com2->total_a_pagar;
        $response['a'] = number_format($com1->total_a_pagar+$com3->notap_total,2,'.','');
        ...
        $this->db->where('YEAR(fecha_de_emision)',date('Y'));         
        $this->db->where_not_in('tipo_documento_id',$doc);
        $this->db->where('anulado',0); 
        $com1 = $this->db->get('comprobantes')->row(); 
        ...
        $this->db->where('YEAR(notap_fecha)',date('Y'));      
        $this->db->where('notap_estado',1); 
        $com3 = $this->db->get('nota_pedido')->row(); 
        //$response['b'] = $com1->total_a_pagar+$com3->notap_total-$com2->total_a_pagar;
        $response['b'] = number_format($com1->total_a_pagar+$com3->notap_total,2,'.','');
       ...
       // $response['c'] = $com1->total_a_pagar-$com2->total_a_pagar;
        ...
        ////TOTAL CLIENTES
        $this->db->where('eliminado_cliente',0);
        $cli = count($this->db->get('clientes')); 
        $response['d'] = $cli;
        ...
        while($mes>=$mes_final){    
            ...
            $resv = number_format($com1->total_a_pagar+$com3->notap_total,2,'.','');    
            ...
            //$resc = $com1->total_a_pagar-$com2->total_a_pagar;
             $resc = number_format($com1->total_a_pagar-$com2->total_a_pagar,2,'.','');
    ...
    {        
        ...
        $this->db->select('c.*,cli.ruc,cli.razon_social,d.codigo,m.abrstandar,m.moneda,clit.codigo as tip_doc_cli,n.codigo as codigo_detraccion');
        ...
        $this->db->join('tipo_detracciones as n','n.id=c.tipo_detraccion_id','left');
        ...
        $datos['cabecera']['tipo_operacion'] = ($comprobante->detraccion == 1) ? '1001':$comprobante->tipo_operacion;
        ...
        $datos['cabecera']['observaciones'] = $comprobante->notas;

         //envio detraccion en off

        $datos['Detraccion']['Estado'] = 'off';

         //para saber si hay un pago al credito
        $creditofilt = array(2);
        //traemos los datos de los pagos
        $pagos_comprobantes = $this->db->select('cp.*,tp.*')
                 ->from('comprobante_pagos cp')
                 ->join('tipo_pagos tp','cp.tipo_pago_id=tp.id')
                 ->where('comprobante_id',$id)
                 ->where_in('cp.tipo_pago_id',$creditofilt)
                 ->order_by('fecha_cuota')
                 ->get()
                 ->result();
                 //var_dump($pagos_comprobantes);exit;
                 if (!empty($pagos_comprobantes)) {
                     $datos['detalle_pagos'] =[];
              $j=1;
              $totalcredito=0;
               foreach ($pagos_comprobantes as $p) {
              ///$datos_pagos['id'] = $p->id;
              //$datos_pagos['observaciones'] = $p->observaciones;
             // $datos_pagos['comprobante_id'] = $p->comprobante_id;
              $datos_pagos['tipo_pago'] = $p->tipo_pago;
              if ($comprobante->detraccion == 1 || $comprobante->retencion == 1) {
                  $datos_pagos['monto'] = $p->monto_neto;
              } else{
                $datos_pagos['monto'] = $p->monto;
              }
              $datos_pagos['fecha_cuota'] = $p->fecha_cuota;
              $datos_pagos['N°cuota'] = "Cuota00".$j;
             // $datos_pagos['estado'] = $p->estado;
              /*validacion que tome monto neto en los casos de retencion o detraccion  */
              if ($comprobante->detraccion == 1 || $comprobante->retencion == 1) {
                  $totalcredito = $totalcredito+$p->monto_neto;
              }else{
                $totalcredito = $totalcredito+$p->monto;
              }
              array_push($datos['detalle_pagos'],$datos_pagos);
              $j++;
                     }
                     $datos['Pago']['FormaPago']='Credito';
                     $datos['Pago']['Monto']=$totalcredito;                    
                 }
                 else
                 {
                       $datos['Pago']['FormaPago']='Contado';
                 }

          //ALEXANDER FERNÁNDEZ 23/12/2021
        $datos['Detraccion']['Estado'] = ($comprobante->detraccion == 1)?"on":"off";
        $datos['Detraccion']['TotalDetraccion'] = $comprobante->total_detraccion;
        $datos['Detraccion']['PorcentajeDetraccion'] = $comprobante->porcentaje_de_detraccion;
        $datos['Detraccion']['NumeroCuentaDetraccion'] = $comprobante->numero_cuenta_detraccion;             
        $datos['Detraccion']['CodigoDetraccion'] = $comprobante->codigo_detraccion;
        $datos['Detraccion']['CodigoMedioPago'] = '001';

          //ALEXANDER FERNÁNDEZ 23/12/2021
        $datos['Retencion']['Estado'] = ($comprobante->retencion == 1)?"on":"off";
        $datos['Retencion']['TotalRetencion'] = $comprobante->total_retencion;
        $datos['Retencion']['PorcentajeRetencion'] = $comprobante->porcentaje_de_retencion;
        ...
        ////DATOS ITEMS 
            ...
            foreach($items as $i){
                ...
                if($i->precio_base == $i->importe){
                    $datos_item['base'] = $i->total;
                    $datos_item['precio_unitario'] = round(($i->total/$i->cantidad)*(1+$igv),2);
                    $datos_item['valor_venta'] = $i->total;
                    $datos_item['impuestos'] = $i->total*$igv;
                    $datos_item['igv'] = $i->total*$igv;
                }else{
                    ...
                    $datos_item['impuestos'] = (intval($i->codigo)>=20)?0:round($i->total-($i->total/(1+$igv)),2);
                    $datos_item['igv'] = (intval($i->codigo)>=20)?0:round($i->total-($i->total/(1+$igv)),2);
                }         
                $datos_item['porcentaje_igv'] = $igv*100;                                
                //$datos_item['valor_unitario'] = (intval($i->codigo)>10)?0:round($i->importe/(1+$igv),10);   
                $datos_item['valor_unitario'] = (intval($i->codigo)>=20)?round(($i->total/$i->cantidad),10):round(($datos_item['base']/$i->cantidad),10);    
    ...
    public function getDatosAnulacion()
    {
        ...
        $rsProductos = $this->db->from('items ite')
                                   ...
                                   ->join('productos pr','pr.prod_id = ite.producto_id')
        ...
        //validacion para que devuelva el stock si es un producto kit
            if ($rsProducto->prod_tipo_insumo == 2) {
               //es un producto kit
                  
               // consulto todos los productos kit 
               $rsProductoss = $this->db->from('producto_presentaciones prp')
                                                    ->join('productos pro','prp.prod_c_id =  pro.prod_id')
                                                    ->where('prp.prod_id',$rsProducto->producto_id)
                                                    ->where('prp.estado',ST_ACTIVO)
                                                    ->get()
                                                    ->result();
                                   //var_dump($rsProductoss);exit;
               foreach ($rsProductoss as  $value2) {
               
                 $concepto = 'ANULACION COMPROBANTE - RESTAURAR / '.$rsProducto->prod_nombre;
                                         //$this->productos_model->salidaStock($value->prod_c_id,$cantidades[$i]*$value->cantidad,$concepto,$rsProducto->prod_id);
                 
                        $this->productos_model->ingresarStock($value2->prod_c_id,$value2->cantidad*$rsProducto->cantidad,$concepto,$rsProductos->id,strtoupper($rsProducto->serie),$rsProducto->numero); 
            ...
            $this->productos_model->ingresarStock($rsProducto->producto_id ,$rsProducto->cantidad, $concepto = 'ANULACION - RESTAURAR STOCK',$comprobante,$rsProducto->serie,$rsProducto->numero);
        }
    ...
    public function pdf($comprobante_id){        
       ...
       // $certificado = $this->ObtenerCertificado($rsEmpresa->ruc,$rsComprobante->codigo,$rsComprobante->serie,$rsComprobante->numero);    
        ...
        //var_dump($data);exit;
        $this->pdf->loadHtml($html);
        $this->pdf->setPaper(array(0,0,100,$ticketHeight), 'portrait');
        $this->pdf->render();
        $tipo_documento_descargar = str_replace(" ","_",$rsComprobante->tipo_documento);
        $this->pdf->stream("$tipo_documento_descargar-$rsComprobante->serie-$rsComprobante->numero.pdf",
            array("Attachment"=>0)
  ...
  public function buscar_documento_s(){
    ...
    switch ($_POST['tipo_documento_id']) {
        case ST_NOTA_PEDIDO:
            ...
            $documento = $this->db->select('np.notap_id documento_id, np.notap_correlativo nDocumento, np.notap_fecha fecha_de_emision,cli.razon_social razon_social_cliente')
                                          ->from('nota_pedido np')
        ...
        case ST_PROFORMA:
            ...
            $documento = $this->db->select('pf.prof_id documento_id, pf.prof_correlativo nDocumento, pf.prof_doc_fecha fecha_de_emision,cli.razon_social razon_social_cliente')
                                          ->from('proformas pf')
        ...
        case ST_COMPROBANTE:
            ...
            $documento = $this->db->select('com.id documento_id, concat_ws("-", com.serie, com.numero) nDocumento, com.fecha_de_emision fecha_de_emision,cli.razon_social razon_social_cliente',FALSE)
