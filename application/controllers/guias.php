...
class Guias extends CI_Controller
{
    ...
    public function guardarGuia()
    {
        ...
        if($_POST['transporte_ruc'] == '')
        {
            $error['transporte_ruc'] = 'falta ingresar tipo conductor destinatario';
        } 
        if($_POST['transporte_razon_social'] == '')
        {
            $error['transporte_razon_social'] = 'falta ingresar tipo conductor destinatario';
        } 
        if($_POST['conductor_documento'] == '')
        {
            $error['conductor_documento'] = 'falta ingresar tipo conductor destinatario';
        } 
        if($_POST['conductor_ruc'] == '')
        {
            $error['conductor_ruc'] = 'falta ingresar tipo conductor destinatario';
        }
        if($_POST['conductor_razon_social'] == '')
        {
            $error['conductor_razon_social'] = 'falta ingresar tipo conductor destinatario';
        }  
        if($_POST['transporte_documento'] == '')
        {
            $error['transporte_documento'] = 'falta ingresar tipo transporte destinatario';
        }
        ...
        if($_POST['numero'] == '')
        {
         ...
         // validacion para el ubigeo
        if($_POST['ubigeo_partida'] == '')
        {
            $error['ubigeo_partida'] = 'seleccione ubigeo de partida';
        } 
        if($_POST['ubigeo_llegada'] == '')
        {
            $error['ubigeo_llegada'] = 'seleccione ubigeo de llegada';
        } 

       /*validacion para que se ingrese un ubigeo valido*/
           $num_ubigeo = substr($_POST['ubigeo_llegada'],0,6);
           $row = $this->db->from('ubigeo_inei')
                           ->where("ubigeo", $num_ubigeo)
                           ->get()
                           ->row(); 
                          // var_dump($row);exit;
           if (!$row) {
               $error['ubigeo_llegada'] = 'ubigeo de llegada invalido';
                $msg1 = 'ubigeo de llegada invalido';
           }

        
        if(count($error) > 0)
        {
            $data = ['status'=>STATUS_FAIL,'tipo'=>1, 'errores'=>$error,'msg'=>$msg1];
            sendJsonData($data);
            exit();
        }  
        ...
        if($_POST['numero_factura']==''){
             ...
             // var_dump($dato_prod->prod_tipo_insumo);exit;
                    if($dato_prod->prod_tipo_insumo==1 || $dato_prod->prod_tipo_insumo==0 ){  //VALIDACION PARA TODOS LOS PRODUCTOS QUE NO SON KIT
                        if($_POST['guiaId'] == ''){
                        ...
                        $itemProdu = $this->db->from('guia_detalles')
                                              ->where('producto_id',$idproduc[$i])                    
                                              ->where('guia_id',$_POST['guiaId'])
                                              ->get()
                                              ->row();
                                              if ($itemProdu->producto_id != 0) {
                                              if($cantidades[$i]==0 OR $cantidades[$i]> ($prod_stock + ($itemProdu->cantidad))){
                        sendJsonData(['status'=>STATUS_FAIL, 'msg'=>'No hay stock disponible de '.$descripcion[$i]]);
                                        exit();}}
                    }
                }else{
                    ...
                    $rsProductoskit = $this->db->from('producto_presentaciones prp')
                            ...
                            foreach ($rsProductoskit as  $value) {
                                
                                $prod_stock_kit = $this->productos_model->getStockProductos($value->prod_c_id,$value->prod_almacen_id);  
                                  
                                    if(($cantidades[$i]*$value->cantidad == 0) OR ($cantidades[$i]*$value->cantidad >$prod_stock_kit)){
                                        sendJsonData(['status'=>STATUS_FAIL, 'msg'=>'No hay stock disponible de '.$prod_stock_kit.'/'.$value->prod_nombre]);
                                        exit();
                                    }
    ...
    public function descargarPdf($idGuia)
    {
        $rsGuia = $this->db->select("gmts.descripcion modalidad,gmt.*,guia.*",FALSE)
                           ...
                           //->join("ubigeo_inei as upar","guia.ubigeo_partida=upar.ubigeo")
                           //->join("ubigeo_inei as ulle","guia.ubigeo_llegada=ulle.ubigeo")
                           ->where("guia.id", $idGuia)
                           ->get()
                           ->row();
       

               // nueva consulta para agregar el codigo de ubigeo
                $rsubigeopar = $this->db->from("ubigeo_inei")
                                     ->where("ubigeo",$rsGuia->ubigeo_partida)
                                     ->get()
                                     ->row();
                      //var_dump($rsubigeopar);exit;               
                $rsGuia->ubigeo_partida_nom = str_replace("|"," - ",$rsubigeopar->localidad);   
                
                // para la llegada
                $rsubigeolleg = $this->db->from("ubigeo_inei")
                                     ->where("ubigeo",$rsGuia->ubigeo_llegada)
                                     ->get()
                                     ->row();
                $rsGuia->ubigeo_llegada_nom = str_replace("|"," - ",$rsubigeolleg->localidad);
  
 
                    //var_dump($rsGuia);exit;
        /*formateamos fecha*/
        ...
        $filename = 'GUIA '.$rsGuia->guia_serie.'-'.$rsGuia->guia_numero;
    ...
    public function getDatosXML(){    
        ...
        $guiaDetalle = $this->db->from("guia_detalles as guiad")
                                ->join("productos as prod", "prod.prod_id = guiad.producto_id","left")
                                ->join("medida med", "prod.prod_medida_id = med.medida_id","left")
